...
export default function FixedBottomForm() {
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      // 마지막 섹션에 가까워지면 자동으로 펼치기 (90% 스크롤 시)
      const scrollPercentage = (scrollTop + windowHeight) / documentHeight;
      const nearBottom = scrollPercentage > 0.9;
  ...
  }, [isNearBottom, isExpanded, isMobile]);
  ...
  return (
    <div className={`fixed bottom-0 left-0 right-0 bg-gradient-to-r from-blue-600 to-purple-600 shadow-2xl z-50 transition-transform duration-300 ease-in-out ${
      !isExpanded ? 'md:transform-none' : ''
    }`}>
      {/* 모바일 상단 탭 (닫힌 상태에서 보이는 부분) */}
      <div 
        className={`md:hidden cursor-pointer transition-all duration-300 ${
          isExpanded ? 'bg-opacity-0' : 'bg-white bg-opacity-10 backdrop-blur-sm'
        }`}
      ...
      >
        <div className="flex items-center justify-center py-3 px-4">
          <div className="flex items-center space-x-2">
            ...
            <span className="text-white font-bold text-sm">원클릭 상담 신청</span>
          </div>
      ...
      <div className={`transition-all duration-300 ease-in-out overflow-hidden ${
        isExpanded || !isMobile ? 'max-h-96 opacity-100' : 'max-h-0 opacity-0 md:max-h-96 md:opacity-100'
      }`}>
